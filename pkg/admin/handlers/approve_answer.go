...
func (h *AdminHandler) ApproveAnswer(w http.ResponseWriter, r *http.Request) {
	...
	log.Debug().Uint32("id", id).Msg("approving answer")

	_, err = h.qadb.ApproveAnswer(ctx, id, user.Id)

	if err != nil {
		log.Err(err).Msg("approving answer")
